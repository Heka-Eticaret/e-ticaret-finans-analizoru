...
import { 
  ...
  FileText, BarChart2, ChevronDown, ChevronRight, Wallet, CheckCircle, XCircle, LogOut, Trash2, Download, FileJson,
...
function App() {
    ...
    if (storedExpenses) {
      try {
        setFixedExpenses(JSON.parse(storedExpenses));
         ...
         // Fall back to initial expenses if parsing fails
         setFixedExpenses(INITIAL_EXPENSES);
      }
    } else {
      // No expenses in localStorage — use initial expenses from the JSON seed and persist them
      setFixedExpenses(INITIAL_EXPENSES);
      try {
        localStorage.setItem('ecommerce_expenses', JSON.stringify(INITIAL_EXPENSES));
      } catch (e) {
        console.warn('Unable to persist initial expenses to localStorage');
      }
    }
  }, []);
  ...
  const overviewMetrics = useMemo(() => {
    // Pass the actual channel filter. calculateMetrics handles setting fixed expenses to 0
    // if platformFilter is not 'all'.
    return calculateMetrics(overviewData, fixedExpenses, globalPeriod, globalChannelFilter);
  }, [overviewData, fixedExpenses, globalPeriod, globalChannelFilter]);

  // Calculate Rates for Overview
  const grossMarginRate = useMemo(() => {
    if (overviewMetrics.totalRevenueExVAT <= 0) return 0;
    return (overviewMetrics.totalGrossProfit / overviewMetrics.totalRevenueExVAT) * 100;
  }, [overviewMetrics]);

  const netMarginRate = useMemo(() => {
    if (overviewMetrics.totalRevenueExVAT <= 0) return 0;
    return (overviewMetrics.totalNetProfit / overviewMetrics.totalRevenueExVAT) * 100;
  }, [overviewMetrics]);
  ...
  const categoryStats = useMemo(() => {
      ...
      filtered.forEach(row => {
          if (row.SiparisStatusu !== 'İade Edildi') {
              ...
              if (!stats[group].products[prodName]) {
                  stats[group].products[prodName] = { qty: 0, revenue: 0 };
              }
              stats[group].products[prodName].qty += row.UrunAdedi;
              stats[group].products[prodName].revenue += row.SiparisTutari;
          }
      });
  ...
  const channelSearchStats = useMemo(() => {
    ...
    const matches = filtered.filter(row => 
        (row.UrunAciklamasi && row.UrunAciklamasi.toLocaleLowerCase('tr-TR').includes(lowerTerm)) ||
        (row.UrunKodu && String(row.UrunKodu).toLocaleLowerCase('tr-TR').includes(lowerTerm))
    );
    ...
    matches.forEach(row => {
        ...
        if (row.SiparisStatusu === 'İade Edildi') {
            stats[key].returnedQty += (row.UrunAdedi || 0);
            stats[key].returnedRevenue += (row.SiparisTutari || 0);
        } else {
            stats[key].deliveredQty += (row.UrunAdedi || 0);
            stats[key].deliveredRevenue += (row.SiparisTutari || 0);
        }
    });
                    ...
                    {activeTab === 'overview' && (
                        <>
                            ...
                            <div className="relative">
                                <select 
                                    value={globalChannelFilter}
                                    onChange={(e) => setGlobalChannelFilter(e.target.value)}
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden mt-8">
                        ...
                        {channelSearchTerm && (
                            <div className="overflow-x-auto">
                                <table className="min-w-full text-sm">
                                    ...
                                    <tbody className="divide-y divide-slate-100">
                                        {channelSearchStats.length > 0 ? (
                                            channelSearchStats.map((item, idx) => (
                                                <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                                    ...
                                                    <td className="px-6 py-3 text-right font-bold text-slate-800 bg-slate-50/50">
                                                        {formatCurrency(item.deliveredRevenue - item.returnedRevenue)}
                                                    </td>
                                                </tr>
                                            ))
                                        ) : (
                        ...
                        {!channelSearchTerm && (
                            <div className="p-8 text-center text-slate-400 text-sm">
                                Sonuçları görmek için yukarıdaki alandan ürün araması yapın.
                            </div>
                        )}
                    </div>
                </div>
